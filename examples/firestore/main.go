...
fmt.Printf("✓ Current quota: %d/%d seconds used\n", usage.Used, usage.Limit)

// Example: Consume quota
err = manager.Consume(ctx, userID, "audio_seconds", 100, goquota.PeriodTypeMonthly)
if err != nil {
log.Fatalf("Failed to consume quota: %v", err)
}
fmt.Println(" Consumed 100 seconds")

// Example: Check quota again
usage, err = manager.GetQuota(ctx, userID, "audio_seconds", goquota.PeriodTypeMonthly)
if err != nil {
log.Fatalf("Failed to get quota: %v", err)
}
...
err = manager.ApplyTierChange(ctx, userID, "scholar", "fluent", "audio_seconds")
if err != nil {
