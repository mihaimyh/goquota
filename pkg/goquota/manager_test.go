...
// Consume 100 seconds
err = manager.Consume(ctx, "user1", "audio_seconds", 100, goquota.PeriodTypeMonthly)
if err != nil {
...
err = manager.Consume(ctx, "user1", "audio_seconds", 5000, goquota.PeriodTypeMonthly)
...
err := manager.Consume(ctx, "user1", "audio_seconds", 0, goquota.PeriodTypeMonthly)
if err != nil {
...
err = manager.Consume(ctx, "user1", "audio_seconds", 1, goquota.PeriodTypeMonthly)
if err != goquota.ErrQuotaExceeded {
...
err = manager.Consume(ctx, "user1", "audio_seconds", 1000, goquota.PeriodTypeMonthly)
if err != nil {
t.Fatalf("Consume failed: %v", err)
}
